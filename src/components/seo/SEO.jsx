...
export default function SEO({ title, description, image = '/og-image.jpg', type = 'website', schema }) {
  ...
  const pageTitle = title ? `${title} | ${site.clinicName}` : (seo?.title || `${site.clinicName} | Hindmotor, Uttarpara`);
  const metaDescription = description || seo?.description || 'Premium homoeopathic consultation in Hindmotor, Uttarpara with appointment booking and online consultation requests.';
  const ogImage = image || seo?.image || '/og-image.jpg';
