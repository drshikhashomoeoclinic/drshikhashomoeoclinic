...
export default function AdminCrud({ collectionName, title, richText = false, uploads = false }) {
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
        <form onSubmit={handleSubmit} className="rounded-[2rem] bg-white p-6 shadow-glass">
          <div className="grid gap-4">
            ...
            <label className="font-semibold">Image URL<input className="admin-input mt-2" name="image" defaultValue={editing?.image || ''} /></label>
            {uploads && <ImageUploader folder={collectionName} onUploaded={(url) => setEditing((current) => ({ ...(current || {}), image: url }))} />}
